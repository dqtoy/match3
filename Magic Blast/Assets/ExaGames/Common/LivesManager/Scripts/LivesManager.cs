	...
	public class LivesManager : MonoBehaviour {
		...
		public bool ConsumeLife() {
			...
			if(lives > 0) {
				...
				if(HasMaxLives) {
					recoveryStartTime = UnbiasedTime.Instance.Now();
		...
		public void GiveInifinite(int minutes) {
			...
			if(!HasInfiniteLives) {
				...
				infiniteLivesStartTime = UnbiasedTime.Instance.Now();
			}
		...
		private void retrievePlayerPrefs() {
			...
			if(PlayerPrefs.HasKey(INFINITE_LIVES_TIME_SAVEKEY) && PlayerPrefs.HasKey(INFINITE_LIVES_MINUTES_SAVEKEY)) {
				...
				//infiniteLivesStartTime = UnbiasedTime.Instance.NewDate(long.Parse(PlayerPrefs.GetString(INFINITE_LIVES_TIME_SAVEKEY)));
				infiniteLivesMinutes = PlayerPrefs.GetInt(INFINITE_LIVES_MINUTES_SAVEKEY);
			} else {
				infiniteLivesStartTime = DateTime.MinValue;
				//infiniteLivesStartTime = UnbiasedTime.Instance.MinValue();
			...
			if(PlayerPrefs.HasKey(LIVES_SAVEKEY) && PlayerPrefs.HasKey(RECOVERY_TIME_SAVEKEY)) {
				...
				//recoveryStartTime = UnbiasedTime.Instance.NewDate(long.Parse(PlayerPrefs.GetString(RECOVERY_TIME_SAVEKEY)));
			} else {
				lives = MaxLives;
				//recoveryStartTime = DateTime.Now;
				recoveryStartTime = UnbiasedTime.Instance.Now();
		...
		private void stepRecoveryTime() {
			if(HasInfiniteLives) {
				...
				if(remainingSecondsWithInfiniteLives < 0D) {
					...
					//infiniteLivesStartTime = UnbiasedTime.Instance.NewDate(0);
					//infiniteLivesStartTime = UnbiasedTime.Instance.Now();
					notifyLivesChanged();
				}
		...
		public class NotificationSettingsStruct {
			...
			public string AlertBody2 = "One lives have been recovered!";
		}
